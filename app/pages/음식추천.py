...
st.set_page_config(layout="wide", page_title="음식 추천", page_icon=":fork_and_knife:")

genai.configure(api_key=st.secrets["food-recommendation"]["GOOGLE_API_KEY"])
...
setProm = (
    "너의 역할은 내가 주는 조건에 맞는 음식을 3가지 이하로 추천해 주는 거고, "
...
with Prompt_Q:
    with st.form(key="my_form"):
        genre = st.radio(
            "음식 장르 선택", ["미정", "분식", "한식", "양식", "일식", "중식"]
        )
        flavor = st.multiselect(
            "원하는 맛 선택(부정의 경우 앞에 <안> 배치)",
            ["달콤한", "매운", "짠", "신", "안"],
        )
        cook = st.multiselect(
            "조리 방식 선택",
            ["국", "탕", "찌개", "찜", "구이", "볶음", "튀김", "스프", "베이킹"],
        )
        time = st.multiselect(
            "식사시간 선택", ["아침 식사", "점심 식사", "저녁 식사", "간식"]
        )
        stuff = st.multiselect(
            "재료 선택",
            ["밥", "빵", "면", "떡", "과일", "야채", "어류", "고기", "갑각류"],
        )
        deliver = st.multiselect(
            "직접조리 or 배달/포장(조리된 음식 구입)", ["직접요리", "조리된 음식 구입"]
        )
