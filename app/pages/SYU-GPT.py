    ...
    try:
        ...
        if os.path.exists(VECTOR_STORE_PATH):
            ...
            docsearch = FAISS.load_local(
                VECTOR_STORE_PATH, hf, allow_dangerous_deserialization=True
            )
        else:
...
def main():
    ...
    st.sidebar.page_link(
        ...
        help="삼육대학교 공식 사이트로 이동합니다",
    )
    st.sidebar.page_link(
        "https://chat.openai.com/", label="ChatGPT", help="Chat GPT 사이트로 이동합니다"
    )
    st.sidebar.page_link(
        "https://gabean.kr/",
        label="GaBean",
